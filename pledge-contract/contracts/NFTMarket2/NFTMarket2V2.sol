    ...
    IERC721Receiver
{
    ...
    // 新的初始化函数，用于升级时初始化新状态
    // function reinitialize() public onlyOwner reinitializer(3) {
    //     // 版本号自加需要常量
    // }
    ...
    function sellNFT(
        ...
        if (address(NFTIERC721Map[NFTaddress].NFT) == address(0)) {
            IERC721 NFTTemp = IERC721(NFTaddress);
            IERC721Metadata NFTTemp2 = IERC721Metadata(NFTaddress);
            NFTIERC721Map[NFTaddress].NFT = NFTTemp;
            NFTIERC721Map[NFTaddress].name = NFTTemp2.name();
            NFTIERC721Map[NFTaddress].symbol = NFTTemp2.symbol();
        }

        // 断言调用者是 NFT 所有者
        require(
            NFTIERC721Map[NFTaddress].NFT.ownerOf(tokenId) == msg.sender,
        ...
        NFTIERC721Map[NFTaddress].NFT.safeTransferFrom(
            msg.sender,
            address(this),
            tokenId
        );
    ...
    function buyNFT(
        ...
        // 转移 NFT 所有权给买家
        NFTIERC721Map[NFTOrderInfoDetail.sellerNFTaddress].NFT.safeTransferFrom(
            address(this),
            to,
            tokenId
        );
    ...
    // 获取所有有订单的 NFT ERC721 信息列表
    function getAllNFTList() external view returns (NFTERC721Info[] memory) {
        // 由于 mapping 没有长度属性，需要先遍历找出所有有效的地址
        address[] memory allAddresses = new address[](orderInfoIds.length);
        uint256 uniqueAddressCount = 0;

        for (uint256 i = 0; i < orderInfoIds.length; i++) {
            address nftAddress = NFTOrderInfoMap[orderInfoIds[i]]
                .sellerNFTaddress;
            bool isUnique = true;

            for (uint256 j = 0; j < uniqueAddressCount; j++) {
                if (allAddresses[j] == nftAddress) {
                    isUnique = false;
                    break;
                }
            }
            ...
            if (isUnique) {
                allAddresses[uniqueAddressCount] = nftAddress;
                uniqueAddressCount++;
            }
        }

        // 创建一个只包含有效信息的数组
        NFTERC721Info[] memory allNFTInfo = new NFTERC721Info[](
            uniqueAddressCount
        );

        for (uint256 i = 0; i < uniqueAddressCount; i++) {
            allNFTInfo[i] = NFTIERC721Map[allAddresses[i]];
        }
    ...
    // 获取单个 NFT 订单列表
    function getNFTAllOrderList(
        address nftAddress
    ) external view returns (NFTOrderInfo[] memory) {
        uint256 length = orderInfoIds.length;
        uint256 validOrderCount = 0;

        // 先统计符合条件的订单数量
        for (uint256 i = 0; i < length; i++) {
            NFTOrderInfo memory order = NFTOrderInfoMap[orderInfoIds[i]];
            if (
                order.sellerNFTaddress == nftAddress &&
        ...
        // 创建一个只包含符合条件订单的数组
        NFTOrderInfo[] memory validOrders = new NFTOrderInfo[](validOrderCount);
        uint256 index = 0;
