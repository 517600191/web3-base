...
import "./NFTTDAO3ERC721URIStorage.sol";

// 管理地址权限的合约
contract NFTTDAO3V2 is
    Initializable,
    OwnableUpgradeable,
    ...
    IERC721Receiver
{
    ...
    NFTTDAO3ERC721URIStorage private _uriStorage;
    ...
    function initialize(
        ...
        _tokenIdCounter = 1;
        _uriStorage = new NFTTDAO3ERC721URIStorage();
    }
    ...
    /**
     * @dev 合约转移自身持有的 NFT
     * @param to 接收 NFT 的目标地址
     * @param tokenId 要转移的 NFT 的 token ID
     */
    function transferSelfOwnedNFT(address to, uint256 tokenId) external onlyOwner {
        address contractAddress = address(this);
        // 检查合约是否拥有该 NFT
        require(ownerOf(tokenId) == contractAddress, "Contract does not own this NFT");
        // 安全转移 NFT
        _transfer(contractAddress, to, tokenId);
    }
