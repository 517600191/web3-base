...
async function main() {
  ...
  if (network.name == "localhost") {
    storePath = await path.resolve(__dirname, './.cache/NFTTDAO3ProxyLocalhost.json');
    _owner = deployer.address;
  } else if (network.name == "sepolia") {
    storePath = await path.resolve(__dirname, './.cache/NFTTDAO3ProxySepolia.json');
  ...
  if (network.name == "localhost") {
    ...
    // const tx1 = await NFTTDAO3Proxy.safeMint("0xef7b473C547a4c5842c25C924E7513C0aE134Dc4", "./NFTTDAO3.json");
    // await tx1.wait();
    // const tx2 = await NFTTDAO3Proxy.safeBatchMint("0xef7b473C547a4c5842c25C924E7513C0aE134Dc4", 2, "./NFTTDAO3.json");
    // await tx2.wait();
  }
...
/* 
借助 Hardhat 部署 NFTTDAO3 合约步骤说明：
1. 环境准备：
   - 确保已安装 Node.js 和 npm。
   - 在项目根目录下安装 Hardhat 及其依赖：`npm install --save-dev hardhat`。
   - 初始化 Hardhat 项目：`npx hardhat`，按照提示选择配置。
   - 将 NFTTDAO3 合约的 Solidity 文件放置在 `contracts` 目录下。
2. 配置 Hardhat：
   - 编辑 `hardhat.config.js` 文件，配置网络、编译器等信息。
3. 运行部署脚本：
   - 在终端中执行 `npx hardhat run scripts/deploy/NFTTDAO3.js --network <network-name>`，其中 `<network-name>` 是你要部署到的网络名称，如 `localhost`、`rinkeby` 等。
4. 查看结果：
