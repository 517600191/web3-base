...
async function main() {
  ...
  if (network.name == "localhost") {
    storePath = await path.resolve(__dirname, './.cache/NFTAuctionProxyLocalhost.json');
  } else if (network.name == "sepolia") {
    storePath = await path.resolve(__dirname, './.cache/NFTAuctionProxySepolia.json');
  }
  ...
  //拿到升级合约
  const NFTAuctionUpgrade = await ethers.getContractFactory("NFTAuctionV3");

  //升级代理合约
  const NFTActionUpgradeProxy = await upgrades.upgradeProxy(proxyAddress, NFTAuctionUpgrade);
  ...
  await fs.writeFileSync(storePath, JSON.stringify({
    proxyAddress,
    upgradeAddress,
    abi: NFTAuctionUpgrade.interface.format("json"),
  }))
}
