...
async function main() {
  ...
  //拿到升级合约
  const AddressPrivilegesUpgrade = await ethers.getContractFactory("AddressPrivilegesV2");

  //升级代理合约
  const NFTActionUpgradeProxy = await upgrades.upgradeProxy(proxyAddress, AddressPrivilegesUpgrade);
  ...
  await fs.writeFileSync(storePath, JSON.stringify({
    proxyAddress,
    upgradeAddress,
    abi: AddressPrivilegesUpgrade.interface.format("json"),
  }))
}
