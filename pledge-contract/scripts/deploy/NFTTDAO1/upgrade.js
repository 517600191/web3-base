...
async function main() {
  ...
  //拿到升级合约
  const NFTTDAO1Upgrade = await ethers.getContractFactory("NFTTDAO1V3");

  //升级代理合约
  const NFTActionUpgradeProxy = await upgrades.upgradeProxy(proxyAddress, NFTTDAO1Upgrade);
  ...
  // 调用新的初始化函数
  // await NFTActionUpgradeProxy.reinitialize(2);

  // 存储合约信息
  await fs.writeFileSync(storePath, JSON.stringify({
    proxyAddress,
    upgradeAddress,
    abi: NFTTDAO1Upgrade.interface.format("json"),
  }))
}
