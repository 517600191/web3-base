...
async function main() {
  ...
  //拿到升级合约
  const MultiSignatureUpgrade = await ethers.getContractFactory("MultiSignatureV2");

  //升级代理合约
  const NFTActionUpgradeProxy = await upgrades.upgradeProxy(proxyAddress, MultiSignatureUpgrade);
  ...
  await fs.writeFileSync(storePath, JSON.stringify({
    proxyAddress,
    upgradeAddress,
    abi: MultiSignatureUpgrade.interface.format("json"),
  }))
}
